  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto p-4 sm:p-6 animate-in fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {goals.map((g) => (
          <Card key={g.id} className="p-5 space-y-4 hoverEffect">
            <div className="flex items-start justify-between gap-2">
              <div className="space-y-1">
                ...
                <p className="text-xs text-surface-500 dark:text-surface-400">{g.description}</p>
              </div>
              <div className="text-right">
                <div className="text-base font-mono font-bold text-foreground">
                  {g.progressPercent}%
                </div>
                <span className="text-[10px] text-surface-400 uppercase">Progress</span>
              </div>
            </div>

            {/* Why statement */}
            <div className="p-3 rounded-lg bg-surface-50 dark:bg-surface-800/60 border border-surface-200/50 dark:border-surface-700/40 text-xs">
              <span className="font-semibold text-brand-600 dark:text-brand-400">Why this matters: </span>
              <span className="text-surface-600 dark:text-surface-300">{g.why}</span>
            </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="space-y-1">
                  {g.milestones.map((m, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-2 text-xs text-surface-600 dark:text-surface-400"
                    >
